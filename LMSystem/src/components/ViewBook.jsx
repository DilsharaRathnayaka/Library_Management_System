      ...
      <div className="container mx-auto p-4 flex-grow">
        ...
        <div className="card-grid flex flex-wrap justify-center gap-8">
          {books.map((book) => (
            <div
              key={book.id}
              className="card bg-gray-600 text-white p-6 rounded-lg shadow-lg hover:shadow-xl hover:transform hover:-translate-y-2 transition"
            >
              <h3 className="card-title text-2xl font-semibold mb-4">{book.BookName}</h3>
              <p className="card-content mb-2">Author: {book.AuthorName}</p>
              <p className="card-content mb-2">ISBN: {book.ISBN}</p>
              <div className="flex justify-center space-x-4">
                <Link to={`/edit/${book.id}`} className="text-white font-bold hover:text-blue-200 transition">
                  Edit
                </Link>
                <DeleteBook id={book.id} />
              </div>
